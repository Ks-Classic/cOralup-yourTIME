...
### 🔵 並行実装グループA: スタッフLINE認証（コード実装）
- Phase 2: Webhook API実装（`/api/line/staff-webhook`）
- Phase 3: LIFF認証実装（`staff-session`, `staff-auth.ts`）
- Phase 4: スタッフ画面実装（`/staff/home`, `/staff/history`）

### 🔵 並行実装グループB: 本番テスト（手動テスト）
- 07-01: LINE登録テスト
- 07-02: 問診フォームテスト
- 07-03: QRスキャンテスト

### 🔵 並行実装グループC: LINE設定（手動作業）
- スタッフ用LINE公式アカウント作成
- LIFF作成・設定
- リッチメニュー作成
...
### ⚠️ 依存関係
```
グループC（LINE設定）完了後 → グループA（Webhook）デプロイ可能
グループA（認証）完了後 → Phase 5（診断フロー統合）実装可能
グループB（テスト）→ 随時実行可能
```
...
#### 03-staff-qr: QRスキャン画面 🆕 **← 優先実装**

**詳細**: [03-07-QRスキャン画面.md](./03-staff/03-07-QRスキャン画面.md)
**仕様書**: [21-QRコード仕様書.md](../designe/21-QRコード仕様書.md)

|| # | タスク | 詳細 | 状態 | 並行 |
||---|--------|------|------|------|
|| 3.17 | QRスキャン画面作成 | `/staff/scan` ページ作成 | ✅ | B |
|| 3.18 | ナビゲーション追加 | layoutにQRスキャンメニュー追加 | ✅ | B |
|| 3.19 | 診断画面へのルーティング | スキャン成功 → `/staff/diagnosis/[visitId]` | 🔧 | B |
|| 3.20 | 診断画面DB対応 | モックデータ → API取得に変更 | 📋 | B |

**並行グループ**: B=本番テストと並行可能

#### 03-staff-auth: スタッフLINE認証 🆕 **← 優先実装**

**詳細**: [03-06-スタッフLINE認証.md](./03-staff/03-06-スタッフLINE認証.md)
**仕様書**: [28-スタッフLINE認証仕様書.md](../designe/28-スタッフLINE認証仕様書.md)

| # | タスク | 詳細 | 状態 | 並行 |
|---|--------|------|------|------|
| 3.8 | LINE公式アカウント作成 | 「cOralupスタッフ」Messaging APIチャネル | 📋 | C |
| 3.9 | LIFF作成 | 同チャネル内でLIFFアプリ追加（1チャネルで完結） | 📋 | C |
| 3.10 | Webhook API実装 | `POST /api/line/staff-webhook` 友だち追加→profiles作成 | 📋 | A |
| 3.11 | LIFF認証実装 | `POST /api/auth/staff-session` + Cookie発行 | 📋 | A |
| 3.12 | スタッフホーム画面 | `/staff/home` QRスキャン + 履歴メニュー | 📋 | A |
| 3.13 | 対応履歴一覧画面 | `/staff/history` 自分の対応一覧 | 📋 | A |
| 3.14 | 対応詳細画面 | `/staff/history/[sessionId]` 問診・診断結果表示 | 📋 | A |
| 3.15 | 診断時スタッフ紐付け | visits.staff_profile_id 設定（QRスキャン時） | 📋 | - |
| 3.16 | スタッフ認証E2Eテスト | 友だち追加→ログイン→診断→履歴確認 | 📋 | - |

**並行グループ**: A=コード実装, C=LINE設定（手動）, -=依存あり
...
| # | タスク | 詳細 | 状態 | 期限 | 並行 |
|---|--------|------|------|------|------|
| 7.1 | LINE登録テスト | 友だち追加→profiles登録確認 | 📋 | 12/10 | B |
| 7.2 | 問診フォームテスト | 基本情報→問診→QR表示→DB登録確認 | 📋 | 12/10 | B |
| 7.3 | QRスキャンテスト | データ引き継ぎ確認 | 📋 | 12/11 | B |
| 7.4 | 写真撮影テスト | Storageアップロード→DB登録確認 | 📋 | 12/11 | B |
| 7.5 | 診断入力テスト | Auto Save→diagnosis_responses登録確認 | 📋 | 12/12 | B |
| 7.6 | AI分析テスト | Gemini API動作確認 | 📋 | 12/12 | B |
| 7.7 | LINE送信テスト | レポート送信→LINE通知確認 | 📋 | 12/13 | B |
| 7.8 | 通しテスト | 全フロー3回実行 | 📋 | 12/14 | - |
| 7.9 | エラーケーステスト | オフライン、タイムアウト等 | 📋 | 12/14 | - |
| 7.10 | テストデータクリア | 本番前クリーンアップ | 📋 | 12/18 | - |
| 7.11 | 最終リハーサル | 本番想定通しテスト | 📋 | 12/18 | - |

**並行グループ**: B=手動テスト（随時実行可能）, -=依存あり

...
| 機能 | 説明 |
|------|------|
| リアルタイム監視 | 全診断セッションの進行状況を可視化 |
| ブラウザ通知 | 重要イベント発生時にPush通知 |
| アラート表示 | 対応が必要なセッションをハイライト |
| 履歴管理 | 完了済みセッションの確認・検索 |
...
| タブ | パス | 説明 |
|------|------|------|
| リアルタイム | `/admin?tab=realtime` | 進行中セッション監視 |
| 履歴管理 | `/admin?tab=history` | 完了済みセッション一覧（旧 `/admin/visits`） |
| LINE送信 | `/admin?tab=line` | LINE送信状況確認・再送信 |
| スキーマ編集 | `/admin/schema-editor` | 問診・診断項目編集 |
| 分析 | `/admin?tab=analytics` | データ分析（将来実装） |

### 2.2 URL設計

| 画面 | URL | 備考 |
|------|-----|------|
| ダッシュボードTOP | `/admin` | デフォルトはリアルタイムタブ |
| リアルタイム | `/admin?tab=realtime` | |
| 履歴管理 | `/admin?tab=history` | |
| 履歴フィルタ | `/admin?tab=history&staffId=xxx&status=yyy` | |
| LINE送信管理 | `/admin?tab=line` | |
| LINE送信フィルタ | `/admin?tab=line&status=needs_resend` | 要再送信のみ |
| スキーマ編集 | `/admin/schema-editor` | 既存のまま |
...
│   │   ├── StatusSummary.tsx         # サマリーカード
│   │   ├── AlertPanel.tsx            # アラートパネル
│   │   ├── ActiveSessionList.tsx     # 進行中リスト
│   │   ├── ActiveSessionCard.tsx     # 進行中カード
│   │   ├── WorkflowIndicator.tsx     # ワークフローインジケーター
│   │   └── CompletedSessionList.tsx  # 完了済みリスト
│   ├── VisitsHistory/
│   │   ├── index.tsx                 # 履歴管理パネル
│   │   ├── FilterPanel.tsx           # フィルターパネル
│   │   └── VisitCard.tsx             # 履歴カード
│   └── NotificationSettings.tsx      # 通知設定モーダル
├── hooks/
│   ├── useRealtimeStatus.ts          # リアルタイムデータ取得
│   ├── useBrowserNotification.ts     # ブラウザ通知
│   ├── useEventDetection.ts          # イベント検知
│   └── useNotificationSettings.ts    # 通知設定管理
└── lib/
    ├── alerts.ts                     # アラート判定ロジック
    └── notifications.ts              # 通知テンプレート
```
...
- [ ] タブUIコンポーネント作成
- [ ] `/api/admin/realtime-status` API実装
- [ ] `useRealtimeStatus` フック実装
- [ ] 基本的なリアルタイムモニター表示

### Phase 2: UI実装（1日）

- [ ] `WorkflowIndicator` コンポーネント
- [ ] `ActiveSessionCard` コンポーネント

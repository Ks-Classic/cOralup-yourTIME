...
**最終更新: 2025-12-21** （Supabase CLIで実DBスキーマ検証済み）
...
| session_id | VARCHAR UNIQUE | 短縮ID（QRコード用、後方互換） |
...
| error_info | JSONB | エラー情報（デバッグ用） |
| is_test_data | BOOLEAN | テストデータフラグ（本番前削除対象識別用）|
...
#### `ai_prompts` (AIプロンプト設定) ※2025-12-18追加

管理画面からAI分析用プロンプトを編集・切り替えるためのテーブル。

| カラム | 型 | 説明 |
|--------|-----|------|
| id | UUID (PK) | |
| label | TEXT | バージョンラベル（例: "v1.0 初期リリース"） |
| prompt_template | TEXT | システムプロンプト本文（テンプレート変数対応） |
| is_active | BOOLEAN | 有効フラグ（常に1件のみtrue） |
| description | TEXT | 内部メモ |
| variable_config | JSONB | 変数設定（使用する変数のON/OFF等） |
| model_name | TEXT | 使用するGeminiモデル名（デフォルト: gemini-2.5-flash-lite） |
| created_at | TIMESTAMP | 作成日時 |
| updated_at | TIMESTAMP | 更新日時 |

**トリガー**: `is_active = true` に更新すると、他のレコードは自動的に `is_active = false` に設定される。
...
**※注**: 旧 `sessions` テーブルは完DBから削除済み。`visits` テーブルに統合されている。
...
| `20241202000001_lark_webhook_trigger.sql` | **廃止対象**: Lark Webhook トリガー。2026-06-29以降の新規環境では適用しない |
...
| `20241218000004_create_ai_prompts.sql` | AIプロンプト管理テーブル作成 |
| `20241219000000_add_test_data_flag.sql` | テストデータフラグ追加（visits, children） |
| `20241219120000_add_variable_config_to_ai_prompts.sql` | ai_prompts.variable_config追加 |
| `20241219130000_add_model_name_to_ai_prompts.sql` | ai_prompts.model_name追加 |
...
### 10.2 レポートURL: `/report/{visits.id}`

現行実装では、レポート表示APIは `visits.id` を受け取り、`reports.visit_id` でレポート行を取得する。
DBに `reports.visit_id` が存在すれば、レポートURL再発行・LINE再送・後日表示が可能。

`reports.uuid` / `reportUuid` をURL正本にする古い設計は廃止する。

```
レポートURL: /report/{visits.id}
```

### 10.3 外部公開IDを追加する場合の条件

将来、`visits.id` をURLに出さない方針へ変更する場合は、`reports.public_token` のような専用カラムを追加し、以下を満たすこと。

- DBスキーマ、API、LINE送信、E2E、設計書を同時に更新する
- `reports.visit_id` とのユニーク制約を維持する
- 既存 `/report/{visitId}` からの移行方針を明記する

現時点では追加しない。

### 10.4 QRコード用: `visits.session_id` (後方互換)

短縮ID（8文字程度）はQRコード表示用に残置。新規実装では `visits.id` の先頭8文字を使用可能。

### 10.5 ユーザーID: `profiles.id` (UUID)
...
### 10.6 運用ルール（2026-06-29更新）

**新規機能開発時:**
- 必ず `visit_id` を外部キーとして使用
- `session_id` は visitsテーブルの表示用カラムとしてのみ参照
- APIは `visitId` を引数として受け取る（sessionIdは非推奨）
- レポートURLは `/report/{visitId}` を使用する
- Lark Base は運用正本ではない。新規開発・当日監視はアプリ内管理画面を正本とする
...
**レジューム（入力再開）機能:**
- LINE User ID + 未完了の `visit_id` で復元
- `session_id` での検索は行わない

**レガシーコード修正時:**
- `session_id` 依存を発見したら `visit_id` 依存に書き換え
- 既存データの `session_id` は残しておいて問題ない（表示用として活用可能）

---

## 11. テーブル使用状況（2025-12-19時点）

### 11.1 実DBテーブル一覧（Supabase CLIで確認）

| テーブル | テーブルサイズ | 推定行数 | 用途 |
|---------|------------|---------|------|
| `questionnaire_responses` | 56 kB | 38 | 問診回答（正規化版） |
| `line_message_logs` | 104 kB | 3 | LINE送信ログ |
| `diagnosis_categories` | 104 kB | 16 | 診断カテゴリマスタ |
| `ai_prompts` | 128 kB | 23 | AIプロンプト設定 |
| `visits` | 48 kB | 4 | 来場セッション管理（**中心テーブル**） |
| `profiles` | 48 kB | 19 | ユーザー台帳（親・スタッフ） |
| `diagnosis_items` | 72 kB | 53 | 診断項目マスタ |
| `reports` | 72 kB | 14 | レポート管理 |
| `visit_photos` | 48 kB | 6 | 写真管理 |
| `questionnaire_items` | 56 kB | 22 | 問診項目マスタ |
| `children` | 48 kB | 4 | 患者（子ども）マスタ |
| `questionnaire_categories` | 16 kB | 10 | 問診カテゴリマスタ |
| `form_schemas` | 16 kB | 1 | フォームスキーマ定義 |
| `organizations` | 16 kB | 1 | 組織マスタ |
| `ai_analysis_logs` | 8 kB | 0 | AI分析ログ |
| `form_responses` | 8 kB | 0 | フォーム回答 |
| `diagnosis_responses` | 8 kB | 0 | 診断回答（正規化版） |
| `questionnaires` | 8 kB | 0 | 問診票（レガシー/JSONB） |
| `diagnoses` | 8 kB | 0 | 診断結果（レガシー/JSONB） |
| `events` | 8 kB | 0 | イベント管理 |
| `form_fields` | 8 kB | 0 | フォームフィールド |
| `form_schema_versions` | 8 kB | 0 | スキーマバージョン |
| `form_cache` | 8 kB | 0 | フォームキャッシュ |
| `event_form_settings` | 0 bytes | 0 | イベント別フォーム設定 |
| `organization_diagnosis_settings` | 0 bytes | 0 | 組織別診断設定 |
| `event_diagnosis_settings` | 0 bytes | 0 | イベント別診断設定 |

### 11.2 レガシー（後方互換用、正規化版への移行推奨）

| テーブル | 状態 | 移行先 |
|---------|------|--------|
| `questionnaires` | 残置（JSONB形式） | `questionnaire_responses` |
| `diagnoses` | 残置（JSONB形式） | `diagnosis_responses` |

**※注**: 旧 `sessions` テーブルは実DBには存在しない（削除済み）。`visits` テーブルに統合された。

...
> **廃止方針: 2026-06-29**
>
> Lark Base 連携は当日運用・レポート反映の正本から外す。
> 新規開発では使用しない。
> DBに保存された正規化データとアプリ内管理画面を正本とし、既存のLark trigger / Edge Function / `LARK_*` 環境変数はアプリ挙動に影響しないことを確認しながら削除する。
>
> この文書は過去設計の記録として残す。実装TODOは [20-運用正本リファクタリング.md](../TODO/20-運用正本リファクタリング.md) を参照。

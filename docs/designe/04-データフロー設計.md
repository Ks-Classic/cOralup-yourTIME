...
sequenceDiagram
    ...
    P->>API: Webhook (Line ID, Name) -> DB Insert (Profiles)

    Note over P: 問診票入力開始 (基本情報)
    P->>L: 入力内容保存
    
    Note over P: 「次へ（基本情報）」押下
    P->>API: POST /api/profiles, /api/children
    API-->>P: 200 OK
...
*   **Submission Strategy**:
    1.  **友だち登録時**: LINE Webhook経由で `profiles` レコード作成。
    2.  **基本情報**: 「次へ」ボタンで `profiles` 更新 & `children` 作成/更新。
    3.  **問診回答**: 「次へ：QR表示」ボタンで `questionnaire_responses` 保存。

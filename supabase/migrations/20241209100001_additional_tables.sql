...
-- 1. visit_photos (写真管理)
-- ============================================
-- sessionsテーブルを参照（visitsが存在する場合はvisitsも参照可能）
CREATE TABLE IF NOT EXISTS visit_photos (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    visit_id UUID REFERENCES visits(id) ON DELETE CASCADE,
    session_id VARCHAR(10) REFERENCES visits(session_id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS line_message_logs (
    ...
    session_id VARCHAR(10) REFERENCES visits(session_id) ON DELETE SET NULL,

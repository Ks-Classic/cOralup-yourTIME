...
export async function GET(request: Request) {
  try {
    ...
    const visitScope: SQL =
      eventIds.length > 0
        ? inArray(visits.eventId, eventIds)
        : and(gte(visits.createdAt, fromDate), lte(visits.createdAt, toDate))!
    ...
    const filteredVisits = includeTest
      ? allVisits
      : allVisits.filter((v) => !v.isTestData)
    ...
    const diagnosisData = await db
      ...
      .innerJoin(
        diagnosisItems,
        eq(diagnosisResponses.itemId, diagnosisItems.id)
      )
      .innerJoin(
        diagnosisCategories,
        eq(diagnosisItems.categoryId, diagnosisCategories.id)
      )
      .innerJoin(visits, eq(diagnosisResponses.visitId, visits.id))
      .where(
        and(visitScope, includeTest ? undefined : eq(visits.isTestData, false))
      )
    ...
    diagnosisData.forEach((row) => {
      ...
      if (!ageGroupClinical[ageGroup][row.itemQuestion])
        ageGroupClinical[ageGroup][row.itemQuestion] = {}
    ...
    for (let i = 0; i < presentItems.length; i++) {
      for (let j = i + 1; j < presentItems.length; j++) {
        ...
        const phi =
          (a * d - b * c) / Math.sqrt((a + b) * (c + d) * (a + c) * (b + d))
    ...
    const ageGroupBenchmarks: Record<
      string,
    ...
    diagnosisData.forEach((row) => {
      ...
      if (!ageGroupBenchmarks[ageGroup][bm.label]) {
        ageGroupBenchmarks[ageGroup][bm.label] = {
          positive: 0,
          total: 0,
          rate: 0,
        }
      ...
      entry.rate =
        entry.total > 0
          ? Number(((entry.positive / entry.total) * 100).toFixed(1))
          : 0
    })
    ...
    try {
      ...
      console.warn(
        '[Report API] questionnaires table not available, skipping marketing text analysis:',
        e
      )
    }
    ...
    try {
      ...
      aiAnalysis.forEach((row) => {
        ...
        if (
          content.includes('A') &&
          (content.includes('良好') ||
            content.includes('問題なし') ||
            content.includes('年齢相応'))
        ) {
          aiGrades['A']++
        } else if (
          content.includes('C') &&
          (content.includes('要相談') ||
            content.includes('専門') ||
            content.includes('相談'))
        ) {
          aiGrades['C']++
    ...
    // ============================================================
    // 平均年齢
    const agesMonths = filteredVisits
      .map((v) => v.childAgeMonths)
      .filter((a) => a && a > 0) as number[]
    const avgAgeMonths =
      agesMonths.length > 0
        ? agesMonths.reduce((s, v) => s + v, 0) / agesMonths.length
        : 0
    const sdAgeMonths =
      agesMonths.length > 1
        ? Math.sqrt(
            agesMonths.reduce((s, v) => s + Math.pow(v - avgAgeMonths, 2), 0) /
              (agesMonths.length - 1)
          )
        : 0
    ...
    const completionRate =
      totalVisits > 0
        ? Number(((funnel.lineSent / totalVisits) * 100).toFixed(1))
        : 0

    // 主要所見率 Top5
    const findingRates: Array<{
      item: string
      rate: number
      count: number
      total: number
    ...
    Object.values(clinicalEvidence).forEach((cat) => {
      Object.values(cat.items).forEach((item) => {
        ...
        if (abVals) {
          const abnormalCount = abVals.reduce(
            (sum, v) => sum + (item.valueCounts[v] || 0),
            0
          )
          const rate = Number(
            ((abnormalCount / item.totalResponses) * 100).toFixed(1)
          )
          findingRates.push({
    ...
    const lineStats = await db
      ...
      .where(
        eventIds.length > 0
          ? visitIds.length > 0
            ? inArray(lineMessageLogs.visitId, visitIds)
            : sql`false`
          : and(
    ...
    // 診断済みvisit数
    const diagnosisVisitCount = new Set(diagnosisData.map((d) => d.visitId))
      .size
    ...
    const correlationItems = [
      ...new Set(correlationMatrix.flatMap((c) => [c.item1, c.item2])),
    ]
    ...
    return NextResponse.json({
      ...
      executiveSummary: {
        ...
        diagnosisCompletionRate:
          totalVisits > 0
            ? Number(((diagnosisVisitCount / totalVisits) * 100).toFixed(1))
            : 0,
        lineDelivery,
        lineSuccessRate:
          lineDelivery.total > 0
            ? Number(
                ((lineDelivery.success / lineDelivery.total) * 100).toFixed(1)
              )
            : 0,
      ...
      correlationAnalysis: {
        ...
        sampleSizeWarning:
          totalVisits < 30
            ? '⚠️ サンプルサイズ(N<30)が統計的検出力を満たしていない可能性があります'
            : null,
      },
